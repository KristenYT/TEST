...
function getUrlFromValue(value) {
  const raw = typeof value === "string" ? value : value?.link || value?.id || "";
...
function parseDetail(html, link) {
  ...
  return createLinkItem(site, link, title, posterPath, description, {
    childItems: parseEpisodes(html, link, title, posterPath),
  });
}
