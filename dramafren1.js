...
function parseEpisodes(html, detailLink, seriesTitle, posterPath) {
  ...
  while ((match = episodeRegex.exec(html)) !== null) {
    ...
    const item = createLinkItem(site, link, title, posterPath, `${seriesTitle} - ${title}`, {
      episode: episode || undefined,
      playerType: "system",
    });
    item.id = `tv.${site.title}.${getQueryParam(link, "id") || getQueryParam(detailLink, "id")}.${episode || episodes.length + 1}`;
    episodes.push(item);
  }
